export function calculateWeightedScore(verdicts, weights) {
    ...
    const finalScore = Math.round((weightedScores.architect + weightedScores.critic + weightedScores.pragmatist) /
        totalWeight);
    ...
    if (Math.abs(verdicts.architect.score - avgScore) > 15) {
        dissent.push(`Architect ${verdicts.architect.score > avgScore ? 'more optimistic' : 'more critical'}: ${verdicts.architect.reasoning}`);
    }
    if (Math.abs(verdicts.critic.score - avgScore) > 15) {
        dissent.push(`Critic ${verdicts.critic.score > avgScore ? 'more lenient' : 'more harsh'}: ${verdicts.critic.reasoning}`);
    }
    if (Math.abs(verdicts.pragmatist.score - avgScore) > 15) {
        dissent.push(`Pragmatist ${verdicts.pragmatist.score > avgScore ? 'more positive' : 'more negative'}: ${verdicts.pragmatist.reasoning}`);
    }
...
function generateVerdict(score, verdicts, consensus) {
    let verdict = '';
    if (score >= 95) {
...
export function mergeIssues(verdicts) {
    const allIssues = [
...
export function generateSuggestions(verdicts, config) {
    const suggestions = [];
    ...
    if (lowestScorer.score < 70) {
        const verdict = verdicts[lowestScorer.agent];
