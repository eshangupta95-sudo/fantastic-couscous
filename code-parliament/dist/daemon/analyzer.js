...
import { ArchitectAgent, CriticAgent, PragmatistAgent } from '../agents/index.js';
...
export class Analyzer {
    ...
    async analyzeFile(filePath) {
        ...
        try {
            ...
            // Update stats
            const totalCost = architectResult.cost + criticResult.cost + pragmatistResult.cost;
            const totalInput = architectResult.usage.inputTokens +
                criticResult.usage.inputTokens +
                pragmatistResult.usage.inputTokens;
            const totalOutput = architectResult.usage.outputTokens +
            ...
            log.info(`Analyzed ${filePath} in ${Date.now() - startTime}ms, score: ${finalVerdict.finalScore}/100, cost: $${totalCost.toFixed(4)}`);
            return finalVerdict;
        }
        catch (error) {
