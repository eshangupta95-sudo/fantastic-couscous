import { z } from 'zod';
import { SQLiteCache, ProjectHealth, FileVerdict, Alert } from '../cache/index.js';
import { ParliamentDaemon } from '../daemon/index.js';
import { ConflictResolver } from '../resolver/index.js';
...
// Tool handlers
export function createToolHandlers(daemon: ParliamentDaemon) {
  const cache = daemon.getCache();

  return {
    parliament_status: async (): Promise<StatusResponse> => {
    ...
    parliament_verdict: async (input: z.infer<typeof VerdictInputSchema>): Promise<VerdictResponse> => {
    ...
    parliament_debate: async (input: z.infer<typeof DebateInputSchema>): Promise<DebateResponse> => {
    ...
    parliament_ignore: async (input: z.infer<typeof IgnoreInputSchema>): Promise<{ acknowledged: boolean }> => {
      cache.addIgnore(input.file, input.rule, input.reason);
      return { acknowledged: true };
    },

    parliament_config: async (input: z.infer<typeof ConfigInputSchema>): Promise<ConfigResponse> => {
      const updates: any = {};

      if (input.baseUrl || input.model || input.apiKey) {
        updates.api = {};
        if (input.baseUrl) updates.api.baseUrl = input.baseUrl;
        if (input.model) updates.api.model = input.model;
        if (input.apiKey) updates.api.apiKey = input.apiKey;
      }
    ...
    parliament_alerts: async (): Promise<{ alerts: Alert[]; cleared: boolean }> => {
      const alerts = cache.getAlerts();
      cache.clearAlerts();
