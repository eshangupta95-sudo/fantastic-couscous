...
export class SQLiteCache {
  ...
  constructor(dbPath: string, targetScore: number = 95) {
  ...
  saveVerdict(verdict: FileVerdict): void {
    ...
    stmt.run(
      verdict.filePath,
      verdict.fileHash,
      verdict.architect.score,
      verdict.architect.reasoning,
      JSON.stringify(verdict.architect.issues),
      verdict.architect.confidence,
      verdict.critic.score,
      verdict.critic.reasoning,
      JSON.stringify(verdict.critic.issues),
      verdict.critic.confidence,
      verdict.pragmatist.score,
      verdict.pragmatist.reasoning,
      JSON.stringify(verdict.pragmatist.issues),
      verdict.pragmatist.confidence,
      verdict.finalScore,
      verdict.finalVerdict,
      JSON.stringify(verdict.suggestions)
    );
  ...
  private getTopIssue(verdict: FileVerdict): {
    description: string;
  ...
  getVerdictByHash(filePath: string, fileHash: string): FileVerdict | null {
    ...
    const row = stmt.get(filePath, fileHash) as any;
    if (!row) return null;

    return this.rowToVerdict(row);
  }

  private rowToVerdict(row: any): FileVerdict {
    return {
      id: row.id,
  ...
  getFilesBelowTarget(): FileVerdict[] {
    ...
    const rows = stmt.all(this.targetScore) as any[];
    return rows.map((row) => this.rowToVerdict(row));
  }
  ...
  getProjectHealth(): ProjectHealth {
    ...
    const overall =
      verdicts.reduce((sum, v) => sum + v.finalScore, 0) / verdicts.length;
    const filesAboveTarget = verdicts.filter(
      (v) => v.finalScore >= this.targetScore
    ).length;
    const filesBelowTarget = verdicts.filter(
      (v) => v.finalScore < this.targetScore
    ).length;
  ...
  getDebates(filePath?: string, limit: number = 10): Debate[] {
    let stmt;
    let rows: any[];
  ...
  getIgnores(filePath: string): Array<{ rule: string; reason: string }> {
    const stmt = this.db.prepare(`SELECT rule, reason FROM ignores WHERE file_path = ?`);
    return stmt.all(filePath) as Array<{ rule: string; reason: string }>;
  }

  isIgnored(filePath: string, rule: string): boolean {
    const stmt = this.db.prepare(
      `SELECT 1 FROM ignores WHERE file_path = ? AND rule = ?`
    );
  ...
  getCostHistory(days: number = 30): CostRecord[] {
    const stmt = this.db.prepare(`
      ...
      LIMIT ?
    `);
    return stmt.all(days) as CostRecord[];
  }
